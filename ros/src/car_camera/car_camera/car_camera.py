...
创建一个发布者节点
"""
class Car_Camera_Publisher(Node):
    ...
    def __init__(self, name):
        ...
        self.cv_bridge = CvBridge()   # ROS2节点父类初始化
        self.publisher_ = self.create_publisher(Image, 'image_raw', 10)  # 创建发布者对象（消息类型、话题名、队列长度）
        self.timer = self.create_timer(0.03, self.timer_callback)         # 创建一个定时器（单位为秒的周期，定时执行的回调函数）
    ...
    def timer_callback(self):
        ...
        frame = cv2.imread("/home/ubuntu/py/1.jpg")
        self.publisher_.publish(
        self.cv_bridge.cv2_to_imgmsg(frame, 'bgr8')) # 发布图像消息
...
def main(args=None):                                 # ROS2节点主入口main函数
    ...
    node = Car_Camera_Publisher("topic_webcam_pub")        # 创建ROS2节点对象并进行初始化
