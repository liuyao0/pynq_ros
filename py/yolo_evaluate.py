...
def raw_post_process(output):
    ...
    for i in range(3):
        ...
        # xy, wh, conf = output[i].sigmoid().split((2, 2, dcfg.nc + 1), 4)
        # print(conf.shape)
        conf = (output[i][...,4:].sigmoid()).view(dcfg.na * nx * ny,dcfg.nc+1)
        
        conf_thres = 0.38
        bs = 1  # batch size
        nc = 23  # number of classes
        xc = conf[..., 0] > conf_thres  # candidates
        conf = conf[xc]
        conf[:, 1:] *= conf[:, 0:1]
        conf, j = conf[:, 1:].max(1, keepdim=True)
        j = j[conf.view(-1)>conf_thres]
        res_list = res_list + j.view(-1).tolist()
    return set(res_list)
