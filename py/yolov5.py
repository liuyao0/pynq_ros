...
def new_pre_process(image, model_image_size):
    t1 = time.time()
    image = image[...,::-1]
...
def main(args=None):
    ...
    # 加载模型文件
    overlay.load_model("/home/ubuntu/new_yolov5.xmodel")
    ...
    print('In0 fix point: ',inputTensors[0].get_attr("fix_point"))
    ...
    print('shapeOut2: ',shapeOut2) 
    print('Out0 fix point: ',outputTensors[0].get_attr("fix_point"))
    print('Out1 fix point: ',outputTensors[1].get_attr("fix_point"))
    print('Out2 fix point: ',outputTensors[2].get_attr("fix_point"))
    ...
    for *xyxy, conf, cls in reversed(res):
        cv2.rectangle(im, (int(xyxy[0]),int(xyxy[1])),(int(xyxy[2]),int(xyxy[3])),(0,255,0),1)
    cv2.imwrite(imgpath.split('.')[0]+"_d.jpg",im)
